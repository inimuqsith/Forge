...
| *2026-09-20* | *Cargo Host Leakage & Zero-Host Builds* | *Cargo membaca `~/.cargo/` host saat sandbox membaca root `/`, memicu potensi kegagalan di rootfs murni tanpa internet* | *Menyuntikkan isolasi otomatis `CARGO_HOME="${srcdir}/.cargo_home"` di engine `builder.rs` untuk seluruh build paket Rust (ADR-083)* |
