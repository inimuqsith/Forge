...
62. **ADR-062 (Dependency Graph Pruning & Smart Skip for Installed Nodes):** Menerapkan deteksi paket terpasang di `InstalledDatabase` pada `WavefrontScheduler` untuk melompati kompilasi dependensi yang sudah terpasang dengan versi/rilis yang cocok (`[SKIP]`), melanjutkan perambatan wavefront secara instan, serta menyediakan flag `--deep` (`--rebuild-deps`) dan `--reinstall` bagi pengguna.
63. **ADR-063 (Official Release Tarball for libxml2):** Mengalihkan sumber `libxml2` dari snapshot tag Git mentah ke tarball rilis resmi GNOME (`download.gnome.org`) yang memuat skrip `./configure` mandiri untuk mencegah kegagalan kompilasi.
64. **ADR-064 (Modern libacl Compatibility for GNU tar):** Menambahkan substitusi `sed` untuk me-rename fungsi internal Gnulib `acl_*_file_at` di `src/xattrs.c` pada resep `tar` guna mencegah tabrakan deklarasi fungsi dengan header `libacl` 2.4.0+ (`/usr/include/sys/acl.h`).
65. **ADR-065 (Upstream GNU GCC v16.2.0 SHA256 Checksum Alignment):** Memverifikasi integritas tarball rilis resmi hulu GNU FTP untuk memastikan kelulusan verifikasi integritas unduhan.
66. **ADR-066 (Per-Chunk Stream Idle Timeout vs Total Timeout in SourceDownloader):** Mengganti pembatasan total request `.timeout()` pada `reqwest::Client` dengan `.connect_timeout()` dan membungkus streaming `.chunk()` dengan `tokio::time::timeout()` per-chunk idle timeout di `SourceDownloader` (`downloader.rs`), memastikan berkas berukuran besar (GCC 100MB+, LLVM, Linux Kernel) dapat diunduh tanpa terputus secara prematur selama transfer data aktif streaming.
67. **ADR-067 (Upstream URL & Checksum Realignment for gobject-introspection & libcap-ng):** Memperbaiki jalur direktori URL GNOME 1.86 untuk `gobject-introspection` (`1.86/gobject-introspection-1.86.0.tar.xz`), mengalihkan `libcap-ng` v0.9.6 ke repositori upstream GitHub resmi dengan bootstrapping autotools (`autoreconf -fiv`), dan menyelaraskan nilai SHA256.
68. **ADR-068 (Smart Multi-Ecosystem Auditor Engine & GNU MPC v1.4.1 Bump):** Memperbarui paket GNU `mpc` ke v1.4.1 tarball `.tar.xz`, menambahkan smart scoring domain/version matching di `audit.py` untuk mengeliminasi homonym ecosystem conflict pada Anitya, menyaring tag prerelease/nightly pada GitHub Atom probe, serta mendukung auto-discovery repository GitHub pada `source_urls`.
...
78. **ADR-084 (O(1) In-Memory Pre-Flight Collision Indexing & Shared System Cargo Cache):** Menerapkan `InstalledDatabase::build_ownership_index` pada `db.rs` untuk membangun inverted hash index in-memory (`PathBuf -> (PackageName, VersionSlot)`) yang memangkas waktu collision check dari $O(N \times M)$ disk scanning (~2 menit) menjadi $O(1)$ (<50ms), serta mengotomasi injeksi `CARGO_HOME="/var/cache/forge/cargo"` dengan writable bind mount di Bubblewrap sandbox guna mengeliminasi redundansi pengunduhan crate pada paket-paket Rust.
...
| *2026-09-20* | *Redundant Dependency Re-compilation* | *Kompilasi ulang dependensi akar (glibc, linux-headers) berulang kali saat target berbeda di-install/update* | *Menerapkan Dependency Graph Pruning & Smart Skip Installed Nodes di `WavefrontScheduler` serta flag `--deep` / `--reinstall` (ADR-062)* |
| *2026-09-20* | *Missing configure Script in libxml2* | *libxml2 mengunduh snapshot Git mentah yang tidak memiliki skrip `./configure` bawaan* | *Mengalihkan sumber ke tarball rilis resmi GNOME `download.gnome.org` (ADR-063)* |
| *2026-09-20* | *libacl 2.4.0+ Function Collision in GNU tar* | *GNU tar 1.35 bentrok deklarasi `acl_*_file_at` dengan header `libacl` baru di `/usr/include/sys/acl.h`* | *Menambahkan `sed` renaming fungsi Gnulib internal `tar_acl_*_file_at` di `src/xattrs.c` (ADR-064)* |
| *2026-09-20* | *GCC v16.2.0 Checksum & Downloader Timeout* | *Hash tarball GCC terpotong prematur akibat global timeout 30 detik pada downloader* | *Menerapkan per-chunk idle timeout di `downloader.rs` (ADR-066) dan mengembalikan hash SHA256 asli berkas utuh (ADR-065)* |
| *2026-09-20* | *Outdated URLs in gobject-introspection & libcap-ng* | *URL direktori GNOME `1.82` mismatch pada gobject-introspection dan Red Hat hosting 404 pada libcap-ng* | *Memperbaiki path direktori GNOME `1.86` dan mengalihkan libcap-ng ke GitHub archive dengan `autoreconf` (ADR-067)* |
...
| *2026-09-20* | *LLVM CMake ASM Ccache Option Collision* | *CMake mendeteksi kata pertama CC (ccache) sebagai ASM compiler sehingga memicu /usr/bin/ccache: invalid option -- 'D'* | *Menambahkan `-DCMAKE_ASM_COMPILER=clang` pada LLVM CMake invocation dan mengekspor `ASM` di `builder.rs` (ADR-077)* |
| *2026-09-20* | *LLVM Redundant libunwind Runtime Build Failure* | *Sub-proyek opsional libunwind gagal dikonfigurasi karena dependensi C++ exception ABI yang belum terisolasi di sandbox* | *Menghapus `-DLLVM_ENABLE_RUNTIMES="libunwind"` dari resep LLVM karena stack unwinding GNU/Linux ditangani native oleh `libgcc_s.so` (ADR-078)* |
| *2026-09-20* | *Cargo C-Crates LTO Bitcode Linking Collision* | *Variabel lingkungan CFLAGS -flto=thin menyebabkan rustc gagal me-link bitcode C crate eksternal (ring, zstd-sys) dengan mold* | *Membersihkan flag LTO dan menyuntikkan `-fno-lto` pada CFLAGS/CXXFLAGS/LDFLAGS sebelum pemanggilan `cargo build` di resep `forge` (ADR-079)* |
...
- **Test Suite:** 101 unit & integration tests lulus 100% (`cargo test --workspace`).
